...
type ConfigOptions struct {
    ...
    StaticFieldEvidence        []StaticFieldEvidence // Page-local evidence for selected static fields
...
4. **Filter Fields** - Remove unwanted locations
   ...
   - When static-field evidence is supplied, retain an otherwise-static field
     only when its normalized value multiset and exact occurrence count match
     one evidence row
...
## Evidence-Aware Static Fields

`ConfigOptions.StaticFieldEvidence` is the narrow exception to
`OnlyVaryingFields=true` for a caller that already has approved observations of
an optional static field. Each row contains the raw observed values and their
exact occurrence count. Generate normalizes both the supplied and parsed values
with the same package-owned normalizer; consumers do not pre-normalize or supply
a matching callback.

The analyzer applies evidence after structural squashing, generated-class
normalization, and the minimum-occurrence filter. It retains varying locations
normally, then retains an otherwise-static data location only when its
normalized value multiset and count exactly match an evidence row. This happens
before `findSharedRootSelector` and `expandAllPossibleConfigs`, so unrelated
static locations never enter the combinatorial generation phase. Pagination
locations retain their existing varying-field behavior.

Evidence is scoped to the one input document analyzed by a generation call. It
does not aggregate continuation pages or later detail-page fetches. A row that
matches no location on that document is inert rather than an error.
`ConfigurationsForGQDocumentWithEvidenceReport` returns the indexes of rows that
matched at least one location and rows that matched none, making normalization
or page-shape drift visible. The legacy `ConfigurationsForGQDocument` API
preserves existing behavior and discards the report; nil evidence is an exact
no-op, and no CLI flag is added.

The evidence filter bounds the locations admitted to root discovery and
recursive expansion. It is not semantic authorization: two locations with the
same values and count may both enter expansion. Callers that own domain meaning
must retain their own post-generation authorization and fail-closed checks.
...
(e.g., header vs footer) that share only a single utility class. The exact-half
case is accepted only when each side has one non-shared class from the same
multi-token or BEM modifier family. This covers record-state variants while
keeping unrelated single-token role classes distinct.

Paths may also differ by one semantic `picture` node directly wrapping the
same structurally matched `img`. The analyzer preserves both exact relative
paths and emits their sorted selector union. Other added wrappers remain
structurally distinct and do not merge.
